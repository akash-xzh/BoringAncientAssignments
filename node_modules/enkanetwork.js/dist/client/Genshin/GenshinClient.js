...
class GenshinClient {
    ...
    async getPlayer(uid, language = this.language) {
        ...
        if (cache != undefined)
            return new structs_1.PlayerData(cache, language);
        const data = await this.handler.player(uid, "genshin");
        cacheClient?.set(`player-${uid}`, data);
        return new structs_1.PlayerData(data, language);
    }
    /**
    ...
    async getEnkaHoyoBuilds(username, hash, language = this.language) {
        ...
        Object.keys(data).forEach((characterId) => {
            if (data[characterId].find((build) => build.hoyo_type == 0)) {
     ...
     */
    parseSubstats(substatsIds, language = this.language) {
        if (!substatsIds)
            throw new errors_1.PackageError("The substatsIds parameter is missing");
        if (!this.languages.includes(language))
            throw new errors_1.AssetFinderError("Invalid or not available language.");
        if (substatsIds?.length <= 0)
            throw new errors_1.PackageError("The substatsIds has no values");
        return substatsIds.map((substatId) => new structs_1.Substats(substatId, language));
    }
}
