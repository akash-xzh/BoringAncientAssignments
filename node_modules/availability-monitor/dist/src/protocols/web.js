'use strict';
import got, { TimeoutError as GotTimeoutError, RequestError as GotRequestError } from 'got';
import puppeteer from 'puppeteer';
// @ts-expect-error
import PuppeteerHar from 'puppeteer-har';
import { MonitorError } from './index.js';
export default class WebProtocolHandler {
    async ping(options) {
    ...
    async pingViaPuppeteer(options) {
        const browser = await puppeteer.launch({ headless: true });
        const page = await browser.newPage();
        let har = new PuppeteerHar(page);
        await har.start();
        try {
            const timeout = typeof options.httpOptions.timeout === 'object' ? undefined : options.httpOptions.timeout;
            const res = await page.goto(options.url, { timeout });
    ...
    async pingViaGot(options) {
        const url = options.url;
        const httpOptions = Object.assign({}, options.httpOptions);
    ...
    static up(data, duration = 0, traceroute) {
        return {
            isUp: true,
            duration: duration,
            event: 'up',
    ...
    static down(data, duration = 0, reason, traceroute) {
        return {
            isUp: false,
            duration: duration,
            event: 'down',
    ...
    static timeout(error, duration = 0, traceroute) {
        return {
            isUp: false,
            duration: duration,
            event: 'timeout',
    ...
    static error(error, duration = 0, traceroute) {
        return {
            isUp: false,
            duration: duration,
            event: 'error',
